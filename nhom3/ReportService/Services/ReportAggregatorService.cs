    ...
    ILogger<ReportAggregatorService> logger) : IReportService
{
    ...
    // ─── Internal DTOs (map từ response của service khác) ────────────────────
    private record OrderSummaryDto(int Id, decimal TotalAmount, DateTime CreatedAt);
    private record ProductStatsDto(int Total, int LowStock);
    private record TopProductDto(int ProductId, string ProductName, int TotalSold, decimal Revenue);
}
